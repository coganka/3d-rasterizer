...
// find the barycentric weights alpha, beta, gamma for point P
vec3_t barycentric_weights(vec2_t a, vec2_t b, vec2_t c, vec2_t p) {
    ...
    // the area of the full triangle ABC using cross product
    float area_triangle_abc = (ab.x * ac.y - ab.y * ac.x);
    // weight alpha is the area of subtriangle BCP divided by the area of full triangle
    float alpha = (bc.x * bp.y - bp.x * bc.y) / area_triangle_abc;
    // weight beta is the area of subtriangle ACP divided by the area of full triangle
    float beta = (ap.x * ac.y - ac.x * ap.y) / area_triangle_abc;
    // weight gamma is the the rest
...
void draw_texel(
    ...
    // variables to store the interpolated values of u, v, 1/w for the current pixel
    ...
    if (interpolated_reciprocal_w < get_zbuffer_at(x, y)) {
        ...
        // draw a pixel at pos x,y with the color that comes from the mapped texture
...
void draw_textured_triangle(
    ...
    // flip the v component to account inverted UV coordinates
    v0 = 1.0 - v0;
    v1 = 1.0 - v1;
    v2 = 1.0 - v2;
